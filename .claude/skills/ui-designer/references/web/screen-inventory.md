...
## 최종 현행화: 2026-04-20 (left-panel-collapse) — 전체 패널 숨김 토글 완성

**이전 상태 (제거됨)**: 섹션별 토글 버튼 3개 (프로젝트/세션/툴 통계 각각)
**현재 상태 (신규)**: IDE 스타일 전체 패널 좌우 숨김 토글 (버튼 1개)
...
```
┌─────────────────────────────────────────────────────────┐
│ HEADER (52px)  🔘패널토글 | 로고 | 상태배지 | 날짜필터  │
├─────────────────────────────────────────────────────────┤
│  ERROR BANNER (1px, 평소 숨김)                          │
├─────────────────────────────────────────────────────────┤
│  SUMMARY STRIP (40px)  활성 | 응답 | 오류 | 세션 | 토큰 │
├─────────────────────────────────────────────────────────┤
│  RIGHT PANEL (1fr)                                      │
│  [기본 뷰] or [세션 상세 뷰]                             │
│  (왼쪽 패널 숨김, 전체 너비 차지)                        │
...
**상태**: ✅ 현행 (2026-04-20 좌측 패널 토글 버튼 추가)

### 0-1. 헤더 왼쪽 영역 (.header-left)

| 요소 | 설명 |
|------|------|
| `.btn-panel-collapse` | 패널 숨김/펼침 토글 (신규) |
| 아이콘 | 세로 막대 + 화살표 (숨김: 오른쪽 화살표, 펼침: 왼쪽 화살표) |
| 상태 | `transform: rotate(180deg)` 적용 시 화살표 방향 전환 |
| 크기 | 32x32px |
| 스타일 | `.btn-panel-collapse { background: none; border: none; cursor: pointer; }` |
| Hover | `background: var(--surface-alt); color: var(--accent);` |
| `.logo` | "Claude**Spyglass**" (span은 일반 텍스트색) |
| `.badge-live` | SSE 상태 배지 (●LIVE/OFFLINE) |

### 0-2. 헤더 중앙 영역 (.date-filter)
...
| 요소 | 설명 |
|------|------|
| `.last-updated` | 마지막 갱신 시각 (10px, `--text-dim`) |
