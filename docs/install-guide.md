...
`SPYGLASS_DIR`은 **훅 스크립트가 위치한 디렉토리의 절대 경로**로 바꿔 작성합니다(예: `/Users/alice/spyglass-hooks`). 이 디렉토리 아래에 `hooks/spyglass-collect.sh`가 존재해야 합니다.

### 4.3 권장 설정 (27개 전체 HOOK_EVENTS)

Subagent·Task·Permission·Compact·Worktree 전체 이벤트까지 수집해 분석 품질을 높이려면 다음 설정을 사용합니다:
