...
export type ChatItemKind = 'text' | 'think' | 'action' | 'orphan-result' | 'system' | 'injected' | 'unknown';

/**
 * 하네스 자동 주입 user 메시지의 종류. role='user' 지만 사람이 타이핑한 발화가 아니라
 * Claude Code 하네스가 user 역할로 끼워 넣은 메시지(일반 발화와 시각 구분 대상).
 *   - 'recap'        : 자리 비웠다 돌아왔을 때 주입되는 요약 지시문("The user stepped away...")
 *   - 'caveat'       : 로컬 커맨드 실행 시 붙는 주의문("Caveat: The messages below were generated...")
 *   - 'slash-command': 슬래시 커맨드 입력 래퍼(<command-name>...)
 */
export type InjectedKind = 'recap' | 'caveat' | 'slash-command';

/**
 * user 메시지 본문이 하네스 자동 주입 패턴인지 분류한다(탐지 SSoT — 호출측 재구현 금지).
 * content **전체가** 해당 패턴일 때만 분류한다(앞부분 고정 문구/태그로 판정). 아니면 null(=일반 발화).
 * 주의: 호출측에서 role==='user' 가드를 책임진다(assistant 텍스트가 우연히 같아도 분류 대상 아님).
 */
export function classifyInjectedUserText(text: string): InjectedKind | null {
  const head = text.trimStart();
  // recap — 자리 비움 복귀 요약 지시문. 고정 도입부.
  if (/^The user stepped away and is coming back\. Recap\b/.test(head)) return 'recap';
  // caveat — 로컬 커맨드 주의문. 고정 도입부.
  if (head.startsWith('Caveat: The messages below were generated by the user while running local commands')) {
    return 'caveat';
  }
  // slash-command — 커맨드 래퍼는 항상 <command-name> 으로 연다.
  if (head.startsWith('<command-name>')) return 'slash-command';
  return null;
}

/** injected 종류 → i18n 라벨 키(라벨 매핑 SSoT — InjectedRow·인스펙터 공용, 호출측 재구현 금지). */
export function injectedLabelKey(kind: InjectedKind | undefined): string {
  switch (kind) {
    case 'recap':
      return 'ui:llm-input.chat.injected-recap-label';
    case 'caveat':
      return 'ui:llm-input.chat.injected-caveat-label';
    case 'slash-command':
      return 'ui:llm-input.chat.injected-slash-label';
    default:
      return 'ui:llm-input.chat.injected-recap-label';
  }
}
...
export interface ChatItem {
  ...
  /** injected: 하네스 자동 주입 종류(recap/caveat/slash-command). 그 외 kind 에서는 undefined. */
  injectedKind?: InjectedKind;
...
export function toChatModel(messages: MessageLike[]): ChatItem[] {
  ...
  messages.forEach((m, i) => {
    ...
    if (typeof content === 'string') {
      // 하네스 자동 주입 user 메시지(recap/caveat/슬래시 래퍼)는 사람 발화가 아니므로 말풍선(text)에서
      // 빼내 별도 위계('injected')로 분리한다. array content 는 멀티파트·system-reminder 동반 실발화라
      // 오탐 위험이 있어 제외 — 주입 메시지는 항상 단일 string content 다.
      const injected = role === 'user' ? classifyInjectedUserText(content) : null;
      if (injected) {
        items.push({ kind: 'injected', role, msgIndex: i, text: content, injectedKind: injected, raw: m });
      } else {
...
export function inspectorPayloadOf(item: ChatItem, t: InspectorTFunc): InspectorPayload {
  ...
  switch (item.kind) {
    ...
    case 'injected':
      return {
        title: t(injectedLabelKey(item.injectedKind)),
        meta: `injected · ${idx} · ${formatBytes((item.text ?? '').length)}`,
        text: item.text ?? '',
        raw,
      };
    case 'system':
