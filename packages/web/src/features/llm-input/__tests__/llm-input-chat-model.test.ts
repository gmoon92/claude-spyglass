...
describe('classifyInjectedUserText', () => {
  it('recap 도입부 → recap', () => {
    expect(
      classifyInjectedUserText('The user stepped away and is coming back. Recap in under 40 words, 1-2 plain sentences.'),
    ).toBe('recap');
  });

  it('caveat 도입부 → caveat', () => {
    expect(
      classifyInjectedUserText('Caveat: The messages below were generated by the user while running local commands. DO NOT respond...'),
    ).toBe('caveat');
  });

  it('슬래시 커맨드 래퍼(<command-name> 시작) → slash-command', () => {
    expect(classifyInjectedUserText('<command-name>/clear</command-name>\n<command-message>clear</command-message>')).toBe(
      'slash-command',
    );
  });

  it('선행 공백 무시(trimStart 후 판정)', () => {
    expect(classifyInjectedUserText('\n  <command-name>/commit</command-name>')).toBe('slash-command');
  });

  it('일반 사용자 발화 → null(오탐 없음)', () => {
    expect(classifyInjectedUserText('이 버그 좀 고쳐줘')).toBeNull();
    expect(classifyInjectedUserText('The user guide says to recap weekly.')).toBeNull(); // recap 단어 포함하나 도입부 불일치
  });
});
...
describe('toChatModel', () => {
  ...
  it('하네스 주입 user 메시지(recap)는 text 아니라 injected 로 분리(injectedKind 보존)', () => {
    const recap = 'The user stepped away and is coming back. Recap in under 40 words.';
    const items = toChatModel([{ role: 'user', content: recap }]);
    expect(items).toHaveLength(1);
    expect(items[0]).toMatchObject({ kind: 'injected', role: 'user', injectedKind: 'recap', text: recap });
  });

  it('assistant 가 우연히 같은 문구여도 user 가 아니면 injected 아님(role 가드)', () => {
    const recap = 'The user stepped away and is coming back. Recap in under 40 words.';
    const items = toChatModel([{ role: 'assistant', content: recap }]);
    expect(items[0]).toMatchObject({ kind: 'text', role: 'assistant' });
  });
