 ...
 *  - useSessionLoad: AbortController + 단건 anomaly fetch + setBloatedSysFor 캐시 + session-anomalies-loaded
 *    CustomEvent 디스패치(원본 loadSession 비동기 IIFE :78-99). 세션 변경/언마운트 시 abort.
 *
 * ★순환 가드(§5)★: 본 모듈은 turn-views.js / 루트 session-detail.js facade 를 import 하지 않는다.
 *  anomaly 캐시는 state/anomaly-cache.js(공유 leaf, 순환 없음)만 사용한다.
 *
 * @module features/session-detail/detail-view
 */
import { useEffect } from 'react';
import { setBloatedSysFor } from '../../../assets/js/state/anomaly-cache.js';
...
/** session-anomalies-loaded CustomEvent 디스패치(사이드바·차트·헤더 동기). */
function dispatchAnomaliesLoaded(sessionId: string, a: SessionAnomalies): void {
  document.dispatchEvent(
    new CustomEvent('session-anomalies-loaded', {
      detail: {
        sessionId,
        bloatedSys: a.bloatedSys,
        contextSaturation: a.contextSaturation,
 ...
 *  - 초기(요청 전): session-anomalies-loaded(null) 디스패치로 이전 세션 잔재 제거(원본 :75-77).
 *  - 성공: setBloatedSysFor 캐시 + onAnomalies 콜백 + session-anomalies-loaded(실값) 디스패치.
 *
...
export function useSessionLoad(sessionId: string | null | undefined, opts: UseSessionLoadOptions = {}): void {
  ...
  useEffect(() => {
    ...
    dispatchAnomaliesLoaded(sessionId, { bloatedSys: null, contextSaturation: null, turnCount: null });
    ...
    (async () => {
      try {
        ...
        // SSoT 캐시 → 사이드바·차트·헤더 동일 데이터 참조(원본 :88).
        setBloatedSysFor(sessionId, a.bloatedSys);
        onAnomalies?.(a);
        dispatchAnomaliesLoaded(sessionId, a);
      } catch (e) {
