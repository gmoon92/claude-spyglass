...
describe('BurnRateCard — 골든마스터', () => {
  it('빈 payload → obs-card-empty + no-data 라벨, id=cardBurnRate', () => {
    const html = renderToStaticMarkup(<BurnRateCard payload={null} t={t} />);
  ...
  it('current_total=0 → 빈 상태(원본 early-return 조건)', () => {
    const html = renderToStaticMarkup(
      <BurnRateCard payload={{ buckets: [{ tokens: 5 }], current_total: 0 }} t={t} />,
    );
    expect(html).toContain('obs-card-empty');
  });
  ...
  it('delta_pct<0 → is-down, delta_pct=0/null → flat "—"', () => {
    const down = renderToStaticMarkup(
      <BurnRateCard payload={{ buckets: [{ tokens: 1 }], current_total: 1, delta_pct: -4.5 }} t={t} />,
    );
    expect(down).toContain('obs-card-trend is-down');
    expect(down).toContain('-4.5%');
    const flat = renderToStaticMarkup(
      <BurnRateCard payload={{ buckets: [{ tokens: 1 }], current_total: 1, delta_pct: 0 }} t={t} />,
    );
  ...
  it('yesterday_same_window=0 → sub 빈 문자열(yesterday 라벨 미노출)', () => {
    const html = renderToStaticMarkup(
      <BurnRateCard payload={{ buckets: [{ tokens: 1 }], current_total: 1, yesterday_same_window: 0 }} t={t} />,
    );
...
describe('CacheHealthCard — 골든마스터', () => {
  it('hit_rate_now=null → 빈 상태', () => {
    const html = renderToStaticMarkup(
      <CacheHealthCard payload={{ buckets: [{ hit_rate: 0.5 }], hit_rate_now: null }} t={t} />,
    );
  ...
  it('hit_rate 임계 톤: ≥0.7 is-up / ≥0.3 is-down / <0.3 is-warn', () => {
    const high = renderToStaticMarkup(
      <CacheHealthCard payload={{ buckets: [{ hit_rate: 0.8 }], hit_rate_now: 0.8 }} t={t} />,
    );
    expect(high).toContain('obs-card-trend is-up');
    expect(high).toContain('80.0%');
    const mid = renderToStaticMarkup(
      <CacheHealthCard payload={{ buckets: [{ hit_rate: 0.5 }], hit_rate_now: 0.5 }} t={t} />,
    );
    expect(mid).toContain('obs-card-trend is-down');
    const low = renderToStaticMarkup(
      <CacheHealthCard payload={{ buckets: [{ hit_rate: 0.1 }], hit_rate_now: 0.1 }} t={t} />,
    );
...
describe('LivePulseCard — 골든마스터', () => {
  it('active_count=0 + no last_event → 빈 상태', () => {
    const html = renderToStaticMarkup(
      <LivePulseCard payload={{ active_count: 0, last_event_ts: null }} t={t} />,
    );
  ...
  it('active_count>0 → is-up + count, recent-activity 라벨', () => {
    const html = renderToStaticMarkup(
      <LivePulseCard payload={{ active_count: 3, last_event_ts: 1_700_000_000_000, recent_calls: [1, 2, 3] }} t={t} />,
    );
    expect(html).toContain('obs-card-trend is-up');
  ...
  it('active_count=0 이지만 last_event 있으면 노출(is-up 아님)', () => {
    const html = renderToStaticMarkup(
      <LivePulseCard payload={{ active_count: 0, last_event_ts: 1_700_000_000_000 }} t={t} />,
    );
...
describe('ToolCategoriesCard — 골든마스터(2 모드 + suppressed)', () => {
  it('빈 배열 → no-tool-calls', () => {
    const html = renderToStaticMarkup(<ToolCategoriesCard payload={[]} t={t} />);
  ...
  it('meta-docs 모드: obs-card-meta-docs + obs-meta-row + invocations', () => {
    const html = renderToStaticMarkup(
      <ToolCategoriesCard payload={{ mode: 'meta-docs', items: [{ name: 'reviewer', invocations: 8 }] }} t={t} />,
    );
  ...
  it('meta-docs 빈 items → no-behavior-defs', () => {
    const html = renderToStaticMarkup(
      <ToolCategoriesCard payload={{ mode: 'meta-docs', items: [] }} t={t} />,
    );
  ...
  it('mode=meta-docs 인데 배열 payload → suppressed(렌더 없음, null)', () => {
    const html = renderToStaticMarkup(
      <ToolCategoriesCard payload={[{ category: 'Agent', request_count: 1 }]} mode="meta-docs" t={t} />,
    );
