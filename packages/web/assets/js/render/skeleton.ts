 ...
 */
export function skTableRows(colSpecs: number | Array<string | number>, count = 2) {
...
function _unit(v: string | number | null | undefined) {
  if (v == null) return '100%';
  if (typeof v === 'number') return `${v}px`;
  return String(v);
}

function _styleSize(width: string | number | null | undefined, height: string | number | null | undefined) {
  const parts = [];
