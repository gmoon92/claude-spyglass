...
function emptySvg(width: number, height: number, color: string) {
  const cy = height - 1;
  return (
 ...
 */
export function sparklineBars(values: Array<number|null>, opts: { width?: number; height?: number; color?: string; gap?: number } = {}) {
 ...
 * @returns {string} SVG 문자열
 */
export function sparklineLine(values: Array<number|null>, opts: { width?: number; height?: number; color?: string; fill?: boolean } = {}) {
  const width = opts.width ?? 96;
  const height = opts.height ?? 22;
  const color = opts.color ?? 'currentColor';
