...
/** 도넛 슬라이스 데이터 — type/model/cache 종류별 데이터 생산자(flat-view 등)가 부여하는 필드 합집합. */
interface DonutSlice {
  ...
  tokens?: number;
  _cacheCreation?: number | null;
}

type DonutKind = 'type' | 'model' | 'cache';
...
let donutMode: DonutKind = 'model';
const dataByKind: Record<DonutKind, DonutSlice[]> = { type: [], model: [], cache: [] };
let typeData: DonutSlice[] = dataByKind.type;   // drawDonut/renderTypeLegend가 참조하는 활성 데이터셋
 ...
 * @param {Array} data
 */
export function setSourceData(kind: DonutKind, data: DonutSlice[]) {
...
export function setTypeData(data: DonutSlice[]) { setSourceData('type', data); }
export function setDonutMode(mode: string) {
  donutMode = (['model', 'cache', 'type'] as const).includes(mode as DonutKind) ? (mode as DonutKind) : 'type';
  typeData = dataByKind[donutMode] || [];
}
export function getDonutMode() { return donutMode; }
export function hasSourceData(kind: DonutKind) { return Array.isArray(dataByKind[kind]) && dataByKind[kind].length > 0; }
...
let _modelTokens: Record<string, string> | null = null;
...
function modelColor(model: string | undefined, idx: number, items: DonutSlice[]) {
  if (!_modelTokens) loadModelTokens();
  const tokens = _modelTokens as Record<string, string>;
...
function donutItemKey(d: DonutSlice, _idx: number) {
...
let _cacheTokens: Record<string, string> | null = null;
...
function cacheItemColor(d: DonutSlice) {
  ...
  const ct = _cacheTokens as Record<string, string>;
  ...
  const labelMap: Record<string, string> = {
    'Cached':      ct.read,
    'Uncached':    ct.others,
    'Cache Write': ct.creation,
  };
  return (d.label && labelMap[d.label]) || COLORS.textDim;
}
function donutItemColor(d: DonutSlice, idx: number): string {
  ...
  const typeColors = TYPE_COLORS as Record<string, string>;
  return donutMode === 'model'
    ? modelColor(d.model, idx, typeData)
    : ((d.type && typeColors[d.type]) || COLORS.textDim);
}
function donutItemCount(d: DonutSlice): number {
...
export function drawTimeline() {
  ...
  const w   = (canvas.parentElement?.clientWidth ?? 0) - 32;
  const h   = 100;
  if (w <= 0) return;
  canvas.width  = w * dpr;
  canvas.height = h * dpr;
...
export function drawDonut() {
  const canvas = document.getElementById('typeChart') as HTMLCanvasElement | null;
  if (!canvas) return;
  const dpr  = window.devicePixelRatio || 1;
  const size = 90;
  ...
  if (donutMode === 'cache') {
    ...
    if (!_cacheTokens) loadCacheTokens();
    ctx.fillStyle    = (_cacheTokens as Record<string, string>).read;
    ...
    ctx.textAlign    = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(total >= 1000 ? (total / 1000).toFixed(1) + 'k' : String(total), cx, cy - 3);
    ctx.fillStyle = COLORS.textDim;
