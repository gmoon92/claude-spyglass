...
function handlePostUpdate(): Response {
  ...
  // 3. `bun install` 무조건 실행.
  //    이전 구현은 매니페스트 diff 로 조건부 install 했지만, fail-skip 분기 (afterSha rev-parse
  //    실패 / nested workspace / 작성자가 lockfile 미커밋 등) 가 더 큰 비용 (자식 부팅이
  //    `Cannot find module` 으로 즉사) 을 초래해 분기 자체를 제거. warm cache 시 install 비용은
  //    ~2-5s 로, 업데이트 빈도 대비 무시 가능. 실패 시 self-restart 차단해 옛 코드로 서비스 유지.
  ...
  // 4. 캐시 갱신 — package.json을 다시 읽어 currentVersion 업데이트
  refreshAfterUpdate();
  const cache = getVersionCache();
