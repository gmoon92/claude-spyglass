...
console.log(`🗑️  Deleting data older than ${DAYS} days`);
console.log(`   Cutoff: ${beforeDate}`);
console.log(`   Cutoff timestamp: ${beforeTs}`);
...
console.log(`\n📊 Current data:`);
console.log(`   sessions: ${sessionCount.count.toLocaleString()}`);
console.log(`   requests: ${requestCount.count.toLocaleString()}`);
...
console.log(`\n🎯 To delete (older than ${DAYS} days):`);
console.log(`   sessions: ${oldSessions.count.toLocaleString()}`);
console.log(`   requests: ${oldRequests.count.toLocaleString()}`);
...
console.log(`   ✅ Sessions deleted: ${deleteSessionsResult.changes.toLocaleString()}`);

// requests는 세션과 연결되지 않은 고아 데이터만 별도 삭제
const deleteRequestsResult = db.run('DELETE FROM requests WHERE timestamp < ?', [beforeTs]);
console.log(`   ✅ Requests deleted: ${deleteRequestsResult.changes.toLocaleString()}`);
...
console.log(`\n📊 Remaining:`);
console.log(`   sessions: ${remainingSessions.count.toLocaleString()}`);
console.log(`   requests: ${remainingRequests.count.toLocaleString()}`);
